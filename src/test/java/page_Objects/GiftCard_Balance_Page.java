...
public class GiftCard_Balance_Page extends BasePage{
    public GiftCard_Balance_Page(AndroidDriver driver) {
        super(driver);
    }

    //elements

    private By giftCard_Code_Field=By.id(".box input[role=\"combobox\"]");
    private By datePicker_Button=By.id("calendar");
    private By daysList_DatePicker=By.id(".month-days>span");
    private By nextYear_Button=By.id(".year-arrow .icon.chevron_next_rtl");
    private By nextMonth_Button=By.id(".month-arrow .icon.chevron_next_rtl");
    private By selectedYearName=By.id(".selected-year-name");
    private By selectedMonthName=By.id(".selected-month-name");
    private By checkBalance_Button=By.id("button[gtm=\"לבדיקת היתרה\"]"); ///used also for verify expected screen opened
    private By errorMessage_WrongGiftCardCode=By.id(".error-message.center>span");
    private By whatIsCardExpirationDate_Text=By.id("//span[text()='מה תוקף השובר?']");
    private By whatIsGiftCarddCode_Text=By.id(".bm-input>div>span");
    private By used_GiftCard_Message=By.id(".center>h3");
    private By valid_giftCard_Balance=By.id("//h3[contains(., 'יתרת שובר מעודכנת')]");
